...
class Print_Media extends MY_Controller {
	...
	public function index() {
		$data['contentView'] = "print_media/index";
		$data['title'] = "Program Monitor :: Print Media";
		$data['brand'] = 'Print Media';
	...
	public function load_activity_list() {
		$results = $this -> global_model -> getActivities('Print Media');
		...
		foreach ($results->result() as $activity) {
			...
			$activity_action = "<a href='#' class='btn-xs btn-primary print _media_manual_update' id='" . $activity -> activity_id . "' >Manual Entry</a><a href='#' class='btn-xs btn-info print _media_activity_upload' id='" . $activity -> activity_id . "' >Upload</a>
			<a href='#' class='btn-xs  print _media_activity_source' id='" . $activity -> activity_id . "' >View Source Data</a>";
	...
	public function print_media_cadre() {
		$results = $this -> print_media_model -> print_media_cadre();
		...
		$this -> load -> view('print _media/charts/chart_grouped', $data);
	}

	public function print_media_frequency() {
		$results = $this -> global_model -> getActivities('Print Media');
		$results = $results -> result_array();

		foreach ($results as $activity) {
			if ($activity['activity_name'] == 'Print Media') {
				$query = "SELECT 
...
GROUP BY dates";
		...
		$this -> load -> view('print _media/charts/chart_line', $data);
	}
