...
class Community_Strategy extends MY_Controller {

	function __construct() {
		parent::__construct();
		$this -> load -> model('global_model');
		$this -> load -> model('community_strategy_model');
	}

	public function index() {
		$data['contentView'] = "community_strategy/index";
		$data['title'] = "Program Monitor :: Community Strategy";
		$data['brand'] = 'Community Strategy';
	...
	public function load_activity_list() {
		$results = $this -> global_model -> getActivities('Community Strategy');
		...
		foreach ($results->result() as $activity) {
			...
			$activity_action = "<a href='#' class='btn-xs btn-primary community_strategy_manual_update' id='" . $activity -> activity_id . "' >Manual Entry</a><a href='#' class='btn-xs btn-info community_strategy_activity_upload' id='" . $activity -> activity_id . "' >Upload</a>
			<a href='#' class='btn-xs  community_strategy_activity_source' id='" . $activity -> activity_id . "' >View Source Data</a>";
	...
	public function community_strategy_cadre() {
		$results = $this -> community_strategy_model -> community_strategy_cadre();
		...
		$this -> load -> view('community_strategy/charts/chart_grouped', $data);
	}

	public function community_strategy_frequency() {
		$results = $this -> global_model -> getActivities('Community Strategy');
		$results = $results -> result_array();

		foreach ($results as $activity) {
			if ($activity['activity_name'] == 'Roll out Community Strategy Training') {
				$query = "SELECT 
...
GROUP BY dates";
		...
		$this -> load -> view('community_strategy/charts/chart_line', $data);
	}
