...
class Redistribution extends MY_Controller {

	function __construct() {
		parent::__construct();
		$this -> load -> model('global_model');
		$this -> load -> model('redistribution_model');
	}

	public function index() {
		$data['contentView'] = "redistribution/index";
		$data['title'] = "Program Monitor :: Redistribution";
		$data['brand'] = 'Redistribution';
	...
	public function load_activity_list() {
		$results = $this -> global_model -> getActivities('Redistribution');
		...
		foreach ($results->result() as $activity) {
			...
			$activity_action = "<a href='#' class='btn-xs btn-primary redistribution_manual_update' id='" . $activity -> activity_id . "' >Manual Entry</a><a href='#' class='btn-xs btn-info redistribution_activity_upload' id='" . $activity -> activity_id . "' >Upload</a>
			<a href='#' class='btn-xs  redistribution_activity_source' id='" . $activity -> activity_id . "' >View Source Data</a>";
	...
	public function redistribution_cadre() {
		$results = $this -> redistribution_model -> redistribution_cadre();
		...
		$this -> load -> view('redistribution/charts/chart_grouped', $data);
	}

	public function redistribution_frequency() {
		$results = $this -> global_model -> getActivities('Redistribution');
		$results = $results -> result_array();

		foreach ($results as $activity) {
			if ($activity['activity_name'] == 'Roll out Redistribution Training') {
				$query = "SELECT 
...
GROUP BY dates";
		...
		$this -> load -> view('redistribution/charts/chart_line', $data);
	}
