...
class Health_Talks extends MY_Controller {

	function __construct() {
		parent::__construct();
		$this -> load -> model('global_model');
		$this -> load -> model('health_talks_model');
	}

	public function index() {
		$data['contentView'] = "health_talks/index";
		$data['title'] = "Program Monitor :: Health_Talks";
		$data['brand'] = 'Health_Talks';
	...
	public function load_activity_list() {
		$results = $this -> global_model -> getActivities('Health_Talks Training');
		...
		foreach ($results->result() as $activity) {
			...
			$activity_action = "<a href='#' class='btn-xs btn-primary health_talks_manual_update' id='" . $activity -> activity_id . "' >Manual Entry</a><a href='#' class='btn-xs btn-info health_talks_activity_upload' id='" . $activity -> activity_id . "' >Upload</a>
			<a href='#' class='btn-xs  health_talks_activity_source' id='" . $activity -> activity_id . "' >View Source Data</a>";
	...
	public function health_talks_cadre() {
		$results = $this -> health_talks_model -> health_talks_cadre();
		...
		$this -> load -> view('health_talks/charts/chart_grouped', $data);
	}

	public function health_talks_frequency() {
		$dataSource = $series = $columns = $seriesData = array();
		$results = $this -> global_model -> getActivities('Health_Talks Training');
...
GROUP BY dates";
		...
		$this -> load -> view('health_talks/charts/chart_line', $data);
	}
