...
class Quantification_Forecasting extends MY_Controller {

	function __construct() {
		parent::__construct();
		$this -> load -> model('global_model');
		$this -> load -> model('quantification_forecasting_model');
	}
	...
	public function manual_entry() {
		...
		redirect('quantification_forecasting');
	}
	...
	public function load_activity_list() {
		...
		foreach ($results->result() as $activity) {
			...
			$activity_action = "<a href='#' class='btn-xs btn-primary quantification_forecasting_manual_update' id='" . $activity -> activity_id . "' >Manual Entry</a><a href='#' class='btn-xs btn-info quantification_forecasting_activity_upload' id='" . $activity -> activity_id . "' >Upload</a>
			<a href='#' class='btn-xs  quantification_forecasting_activity_source' id='" . $activity -> activity_id . "' >View Source Data</a>";
	...
	public function quantification_forecasting_cadre() {
		$results = $this -> quantification_forecasting_model -> quantification_forecasting_cadre();
		...
		$this -> load -> view('quantification_forecasting/charts/chart_grouped', $data);
	}

	public function quantification_forecasting_frequency() {
		$results = $this -> global_model -> getActivities('Quantification_Forecasting');
		$results = $results -> result_array();

		foreach ($results as $activity) {
			if ($activity['activity_name'] == 'Roll out Quantification_Forecasting Training') {
				$query = "SELECT 
...
GROUP BY dates";
		...
		$this -> load -> view('quantification_forecasting/charts/chart_line', $data);
	}
