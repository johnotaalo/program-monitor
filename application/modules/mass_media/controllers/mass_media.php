...
class Mass_Media extends MY_Controller {

	function __construct() {
		parent::__construct();
		$this -> load -> model('global_model');
		$this -> load -> model('mass_media_model');
	}

	public function index() {
		$data['contentView'] = "mass_media/index";
		$data['title'] = "Program Monitor :: Mass Media";
		$data['brand'] = 'Mass Media';
	...
	public function load_activity_list() {
		$results = $this -> global_model -> getActivities('Mass Media');
		...
		foreach ($results->result() as $activity) {
			...
			$activity_action = "<a href='#' class='btn-xs btn-primary mass_media_manual_update' id='" . $activity -> activity_id . "' >Manual Entry</a><a href='#' class='btn-xs btn-info mass_media_activity_upload' id='" . $activity -> activity_id . "' >Upload</a>
			<a href='#' class='btn-xs  mass_media_activity_source' id='" . $activity -> activity_id . "' >View Source Data</a>";
	...
	public function mass_media_cadre() {
		$results = $this -> mass_media_model -> mass_media_cadre();
		...
		$this -> load -> view('mass_media/charts/chart_grouped', $data);
	}

	public function mass_media_frequency() {
		$results = $this -> global_model -> getActivities('Mass Media');
		$results = $results -> result_array();

		foreach ($results as $activity) {
			if ($activity['activity_name'] == 'Roll out Mass Media Training') {
				$query = "SELECT 
...
GROUP BY dates";
		...
		$this -> load -> view('mass_media/charts/chart_line', $data);
	}
