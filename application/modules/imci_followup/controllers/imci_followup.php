...
class IMCI_Followup extends MY_Controller {

	function __construct() {
		parent::__construct();
		$this -> load -> model('global_model');
		$this -> load -> model('imci_followup_model');
	}

	public function index() {
		$data['contentView'] = "imci_followup/index";
		$data['title'] = "Program Monitor :: IMCI Follow Up";
		$data['brand'] = 'IMCI Follow Up';
	...
	public function load_activity_list() {
		$results = $this -> global_model -> getActivities('IMCI Follow Up');
		...
		foreach ($results->result() as $activity) {
			...
			$activity_action = "<a href='#' class='btn-xs btn-primary imci_followup_manual_update' id='" . $activity -> activity_id . "' >Manual Entry</a><a href='#' class='btn-xs btn-info imci_followup_activity_upload' id='" . $activity -> activity_id . "' >Upload</a>
			<a href='#' class='btn-xs  imci_followup_activity_source' id='" . $activity -> activity_id . "' >View Source Data</a>";
	...
	public function imci_followup_cadre() {
		$results = $this -> imci_followup_model -> imci_followup_cadre();
		...
		$this -> load -> view('imci_followup/charts/chart_grouped', $data);
	}

	public function imci_followup_frequency() {
		$dataSource = $series = $columns = $seriesData = array();
		$results = $this -> global_model -> getActivities('IMCI Followup');
...
GROUP BY dates";
		...
		$this -> load -> view('imci_followup/charts/chart_line', $data);
	}
