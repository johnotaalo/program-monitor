...
class Procurement extends MY_Controller {
	...
	public function index() {
		$data['contentView'] = "procurement/index";
		$data['title'] = "Program Monitor :: Procurement";
		$data['brand'] = 'Procurement';
	...
	public function load_activity_list() {
		$results = $this -> global_model -> getActivities('Procurement');
		...
		foreach ($results->result() as $activity) {
			...
			$activity_action = "<a href='#' class='btn-xs btn-primary procurement_manual_update' id='" . $activity -> activity_id . "' >Manual Entry</a><a href='#' class='btn-xs btn-info procurement_activity_upload' id='" . $activity -> activity_id . "' >Upload</a>
			<a href='#' class='btn-xs  procurement_activity_source' id='" . $activity -> activity_id . "' >View Source Data</a>";
	...
	public function procurement_cadre() {
		$results = $this -> procurement_model -> procurement_cadre();
		...
		$this -> load -> view('procurement/charts/chart_grouped', $data);
	}

	public function procurement_frequency() {
		$results = $this -> global_model -> getActivities('Procurement');
		$results = $results -> result_array();

		foreach ($results as $activity) {
			if ($activity['activity_name'] == 'Roll out Procurement Training') {
				$query = "SELECT 
...
GROUP BY dates";
		...
		$this -> load -> view('procurement/charts/chart_line', $data);
	}
