...
class ORT extends MY_Controller {

	function __construct() {
		parent::__construct();
		$this -> load -> model('global_model');
		$this -> load -> model('ort_model');
	}

	public function index() {
		$data['contentView'] = "ort/index";
		$data['title'] = "Program Monitor :: ORT";
		$data['brand'] = 'ORT';
	...
	public function load_activity_list() {
		$results = $this -> global_model -> getActivities('ORT');
		...
		foreach ($results->result() as $activity) {
			...
			$activity_action = "<a href='#' class='btn-xs btn-primary ort_manual_update' id='" . $activity -> activity_id . "' >Manual Entry</a><a href='#' class='btn-xs btn-info ort_activity_upload' id='" . $activity -> activity_id . "' >Upload</a>
			<a href='#' class='btn-xs  ort_activity_source' id='" . $activity -> activity_id . "' >View Source Data</a>";
	...
	public function ort_cadre() {
		$results = $this -> ort_model -> ort_cadre();
		...
		$this -> load -> view('ort/charts/chart_grouped', $data);
	}

	public function ort_frequency() {
		$dataSource = $series = $columns = $seriesData = array();
		$results = $this -> global_model -> getActivities('ORT');
...
GROUP BY dates";
		...
		$this -> load -> view('ort/charts/chart_line', $data);
	}
