...
class Outreaches extends MY_Controller {

	function __construct() {
		parent::__construct();
		$this -> load -> model('global_model');
		$this -> load -> model('outreaches_model');
	}

	public function index() {
		$data['contentView'] = "outreaches/index";
		$data['title'] = "Program Monitor :: Outreaches";
		$data['brand'] = 'Outreaches';
	...
	public function load_activity_list() {
		$results = $this -> global_model -> getActivities('Outreaches');
		...
		foreach ($results->result() as $activity) {
			...
			$activity_action = "<a href='#' class='btn-xs btn-primary outreaches_manual_update' id='" . $activity -> activity_id . "' >Manual Entry</a><a href='#' class='btn-xs btn-info outreaches_activity_upload' id='" . $activity -> activity_id . "' >Upload</a>
			<a href='#' class='btn-xs  outreaches_activity_source' id='" . $activity -> activity_id . "' >View Source Data</a>";
	...
	public function outreaches_cadre() {
		$results = $this -> outreaches_model -> outreaches_cadre();
		...
		$this -> load -> view('outreaches/charts/chart_grouped', $data);
	}

	public function outreaches_frequency() {
		$dataSource = $series = $columns = $seriesData = array();
		$results = $this -> global_model -> getActivities('Outreaches');
...
GROUP BY dates";
		...
		$this -> load -> view('outreaches/charts/chart_line', $data);
	}
